...
def bad_visibility(visibility,minimum):
    ...
    if type(visibility) == str:
        return True
    try:
        m = visibility['minimum']
    except:
        m = visibility['prevailing']
...
def bad_ceiling(ceiling,minimum):
    ...
    if type(ceiling) == str:
        if ceiling == 'clear':
            ...
            return True
    tp = ['broken','overcast','indefinite ceiling']
    result = False
    for ce in ceiling:
        if ce['type'] in tp:
            result = result or ce['height']<minimum
    return result
...
def get_weather_report(takeoff,weather):
    ...
    ks = weather.keys()
    result = '2000-04-21T08:00:00-04:00'
    if takeoff.isoformat() in ks:
        return weather[takeoff.isoformat()]
    else:
        for w in ks:
            if utils.str_to_time(w)<takeoff and utils.str_to_time(w)>utils.str_to_time(result):
                result = w
        if result != '2000-04-21T08:00:00-04:00':
...
def list_weather_violations(directory):
    ...
    daycycle = utils.read_json(directory+'/daycycle.json')
    weather = utils.read_json(directory+'/weather.json')
    minimums = utils.read_csv(directory+'/minimums.csv')
    students = utils.read_csv(directory+'/students.csv')
    lessons = utils.read_csv(directory+'/lessons.csv')
    ...
    result = []
    for l in lessons[1:]:
        tt = utils.str_to_time(l[3])
        student = utils.get_for_id(l[0],students)
        pc = pilots.get_certification(tt,student)
        pm = pilots.get_minimums(pc,l[6],l[2]!='',l[5]=='VFR',utils.daytime(tt,daycycle),minimums)
        wc = get_weather_report(tt,weather)
        vi = get_weather_violation(wc,pm)
        if vi != '':
            r = l
            r.append(vi)
            result.append(r)
