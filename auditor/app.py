...
def discover_violations(directory,output):
    ...
    """
    result = [['STUDENT','AIRPLANE','INSTRUCTOR','TAKEOFF','LANDING','FILED','AREA','REASON']]
    v = violations.list_weather_violations(directory)
    if len(v)>0:
        for l in v:
            result.append(l)
        if output != None:
            utils.write_csv(result,output)
        if len(v)>1:
            print(len(v),'violations found.')
        else:
