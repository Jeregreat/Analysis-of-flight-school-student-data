...
def get_certification(takeoff,student):
    ...
    result = -1
    i = 3
    while i<7 and student[i]!='':
        if takeoff >= utils.str_to_time(student[i],tzsource=takeoff):
            result += 1
        i += 1
    return result
...
def get_best_value(data, index, maximum=True):
    ...
    for i in range(1,len(data)):
        if maximum:
            result = max(result,float(data[i][index]))
        else:
            result = min(result,float(data[i][index]))
    return result
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    for r in minimums[1:]:
        ...
        if r[0] == 'Student' and cert>=1:
            match1 = True
        elif r[0] == 'Certified' and cert>=2:
            match1 = True
        elif r[0] == '50 Hours' and cert>=3:
            match1 = True
        elif r[0] == 'Dual' and instructed:
            match1 = True

        if r[1] == 'IMC' and not vfr:
            match2 = True
        elif r[1] == 'VMC' and vfr:
            match2 = True
    
        if r[2] == 'Any':
            match3 = True
        elif r[2] == 'Local':
            if area == 'Pattern' or area == 'Practice Area':
                match3 = True
        elif r[2] == area:
            match3 = True
        
        if r[3] == 'Day' and daytime:
            match4 = True
        elif r[3] == 'Night' and not daytime:
            match4 = True
    ...
    for i in range(4,8):
        if i<=5:
            result.append(get_best_value(rows,i,maximum=False))
        else:
            result.append(get_best_value(rows,i))
    return result
